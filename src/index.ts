...
app.post("/api/videos/:id/like",auth,async(req,res)=>{
    const userId=req.userId;
     if(!userId) {res.status(411).json({message:"Unauthorized"}); return}
    const videoId=req.params.id as string;
    if(!videoId) {res.status(411).json({message:"error"}); return}
...
app.post("/api/playlist",auth,async(req,res)=>{
    const userId=req.userId;
    if(!userId) {res.status(411).json("Unauthorized");return}
